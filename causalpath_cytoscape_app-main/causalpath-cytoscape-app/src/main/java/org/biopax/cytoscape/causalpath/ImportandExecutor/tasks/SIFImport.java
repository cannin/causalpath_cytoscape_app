...
import org.biopax.cytoscape.causalpath.utils.CreateNetwork;
   ...
   public  SynchronousTaskManager synchronousTaskManager;
    ...
    public void writeSIF(File siffile, String fXmlFile, boolean flag) throws ParserConfigurationException, SAXException, IOException, TransformerException {
        ...
        try {
            ...
            if(true)
                try {
                    //CommandExecutor.execute(new TaskIterator(new CreateNetwork(cyServiceRegistrar.getService(CyNetworkFactory.class))),cyServiceRegistrar);
                     CyNetworkFactory cyNetworkFactory = cyServiceRegistrar.getService(CyNetworkFactory.class);
                     SIFCyNetwork = cyNetworkFactory.createNetwork();
                    CyNetworkNaming cyNetworkNaming = cyServiceRegistrar.getService(CyNetworkNaming.class);
                    SIFCyNetwork.getDefaultNetworkTable().getRow(SIFCyNetwork.getSUID())
                             .set("name", cyNetworkNaming.getSuggestedNetworkTitle(networkName));
                    CyNetworkManager cyNetworkManager = cyServiceRegistrar.getService(CyNetworkManager.class);
                    cyNetworkManager.addNetwork(SIFCyNetwork,false);
                    //CommandExecutor.execute("create network=" + networkName,cyServiceRegistrar);
            ...
            SIFCyNetwork = CyNetworkUtils.readCyNetworkFromFile(cyServiceRegistrar, temp,synchronousTaskManager,SIFCyNetwork);
